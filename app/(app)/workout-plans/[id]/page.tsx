...
export default async function WorkoutPlanPage({
  ...
  return (
    <>
      ...
      <div className='-mx-5 flex gap-3 overflow-x-auto px-5 py-5 lg:mx-0 lg:grid lg:grid-cols-4 lg:gap-3 lg:overflow-visible lg:px-0 lg:py-6'>
        {sortedDays.map((day) => (
          <div
            key={day.id}
            className='w-[min(280px,80vw)] shrink-0 lg:w-auto lg:shrink'
          >
            {day.isRest ? (
              <RestDayCard weekDay={day.weekDay} />
            ) : (
              <Link href={`/workout-plans/${id}/days/${day.id}`}>
                <WorkoutDayCard
                  name={day.name}
                  weekDay={day.weekDay}
